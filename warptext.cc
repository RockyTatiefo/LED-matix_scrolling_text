/*
	...
	and then display the text and scrolls it across the led board with the text warping the the beginning when it reaches the
	end
*/
...
// Use to make a server that will recieve text from a client 
...
int main(int argc, char *argv[]) {
	...
	int y_orig = 16;  // Y placement of text
	int speed = 5;	// Scrolling speed
	int sleept; //Sleep time determining scrolling speed
	Color color(255, 255, 0); // Text color
	Color *bgcolor = NULL; // Background color
	...
	// Get text to display from a client
	...
	sleept = 150000.0 / speed; // Sleep for LED scrolling is based off speed
	std::unique_lock<std::mutex> lck(mtx, std::defer_lock);
	
	// Keep scrolling until client disconnects or sends '\n'
	while(scroll){
		lck.lock();
		strcpy(letters, cmsg);
		printf("Displaying '%s' \n", letters);
		...
		int move = rgb_matrix::DrawText(canvas, font, (chain * 32) , y_orig, color, bgcolor, letters); // LED lenght of text

		//If the text pixel lenght is too long, it will not be able to be diplayed properly
		if (move >= (chain * 32)){
			std::cout << "Pixel lenght too long. Try a smaller font or shorter text \n";
			usleep(1000000);
			continue;
		}
		
		int startpos = (chain*32) - move; // Will contain the starting position for the scrolling

		// one round of warp scrolling
		for(int i = 0; i < (chain*32); i++){
			int xpos = startpos - i;
			canvas->Clear();
			rgb_matrix::DrawText(canvas, font, xpos, y_orig, color, bgcolor, letters);
			if(xpos < 0)
				rgb_matrix::DrawText(canvas, font, ((chain*32) + xpos) , y_orig, color, bgcolor, letters);
